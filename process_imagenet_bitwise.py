...
def main():
    ...
    for l in range(total_bits):
        if l in skipped_list:
            data.append(0)
            continue
        file1 = open(dataset_name + '_logs/' + dataset_name + '_final_log_' + model_name + '_' + str(l) + '_bit.txt', 'r')
        # print(file1)
        ...
        print("Bit " + str(l) + " : Non-SCM " + str(faulty_non_avmis) + ", SCM " + str(faulty_avmis))
    ...
    plt.bar(list(range(0, 32)), second_data_list)
    plt.xlabel("Bit Position")
    plt.ylabel("SCM Probability")
    plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter())
    plt.tight_layout()
    plt.savefig(dataset_name + '_' + model_name + '_bit.png')
    # plt.show()
    # print(data_list)
