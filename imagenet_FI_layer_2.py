...
def get_data_path_by_model_name(model_name, path_imagenet_val_dataset):
    if model_name == "ResNet50" or model_name == "ResNet101" or model_name == "ResNet152" or model_name == "VGG16" \
        ...
        return str(path_imagenet_val_dataset) + "/sampled_x_val_299_"
    elif model_name == "NASNetLarge":
        return str(path_imagenet_val_dataset) + "/sampled_x_val_331_"
...
def main():
    ...
    file_object = open('imagenet_logs/imagenet_final_log_' + model_name + '_' + str(layer_no) + '_layer.txt', 'a')
    ...
    x_val = np.load(x_val_path + "1.npy").astype('float32')
    x_val = np.append(x_val, np.load(x_val_path + "2.npy").astype('float32'), axis=0)
    ...
    for i in range(total_injection):
        ...
        file_object.write(str(i) + " : " + str(rand_num) + " : " + str(y_val[rand_num]) + " : " + str(predicted_label) + " : " + str(faulty_prediction))
        file_object.write("\n")
        file_object.flush()
    file_object.close()
    print("Done")
